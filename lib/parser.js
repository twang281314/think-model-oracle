...
module.exports = class OracleParser extends Parser {
  ...
  parseValue(value) {
    if (helper.isString(value)) {
      if (value.match(/^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/)) {
        value = "to_date('" + value + "','yyyy-mm-dd,hh24:mi:ss')"
      } //时间戳转为日期
      else if (/^\d+$/.test(value) && value.length > 14 && isNaN(value) && !isNaN(Date.parse(value))) {
        value = "to_date('" + value + "','yyyy-mm-dd,hh24:mi:ss')"
      } else {
  ...
  parseLimit(selectSql, limit) {
    ...
    const sql = `
      ...
      FROM (SELECT ROWNUM AS rowno,r.*
                ...
                where ROWNUM>0 and ROWNUM <= ${data[1]+data[0]}
                ) table_alias
