...
module.exports = class OracleSchema extends Schema {
  ...
  getSchema(table = this.table) {
    ...
    return debounce.debounce(`getTable${table}Schema`, () => {
      const columnSql = `
            SELECT utc.column_name AS column_name, utc.data_type AS data_type, utc.data_length AS max_length
            , CASE utc.nullable
              WHEN 'N' THEN '否'
              ELSE '是'
            END AS required, utc.data_default AS default_value, ucc.comments AS comments, utc.table_name AS table_name
            , CASE utc.COLUMN_NAME
              WHEN (
                ...
                WHERE (con.constraint_name = col.constraint_name
                  ...
                  AND col.table_name = '${this.parser.parseKey(table)}')
              ) THEN '是'
              ELSE '否'
            END AS is_primary,
            CASE utc.COLUMN_NAME
          ...
          WHERE (utc.table_name = ucc.table_name
            ...
            AND utc.table_name = '${this.parser.parseKey(table)}')
          ORDER BY column_id`;
